...
# Now it is time to clculate the χ² value for each cell
chi_squared <- sum((data[, -1] - expected_values)^2 / expected_values)

# In this step, we will check the degrees of freedom
df <- (nrow(data) - 1) * (ncol(data) - 1)

# Finally, we will [rint the Chi-squared statistic
cat("Chi-squared statistic:", chi_squared, "\n")


## (b)

# We need to calculate the p-value and run the chi-squared test 
...
## We our p-value result we can not reject the null hypothesis because it suggests that there is no significant relationship between the social class of drives and the solicitation of a bribe
...
colnames(standardized_residuals_table) <- c("Class", "Not Stopped", "Bribe requested", "Stopped/given warning")
